...
作为金融分析师，我需要查看市场杠杆率、货币供应比率和利率成本分析的核心指标，以便快速评估当前市场杠杆水平。

**为什么这是最高优先级**: 这些基础杠杆指标是所有后续分析的基础，用户需要首先了解当前市场状况和风险水平。
...
- **FR-001**: 系统必须支持从FINRA获取融资余额统计数据，包括月度数据和历史数据
...
- **FR-005**: 系统必须计算并展示Part1指标：市场杠杆率（Margin Debt / S&P 500总市值）、货币供应比率（Margin Debt / M2）、利率成本分析（Margin Debt vs 利率关系）
- **FR-006**: 系统必须计算并展示Part2指标：杠杆变化率（Margin Debt年同比变化率YoY%）、投资者净资产（现金余额-借方余额-市场缓冲垫）、脆弱性指数（杠杆Z分数-VIX Z分数）
...
- **SC-002**: 系统能够处理并显示至少10年历史数据（从2013年至2023年），支持VIX数据从1990年开始
- **SC-003**: 交互式图表响应时间小于2秒，支持实时缩放、平移和数据点悬停
- **SC-004**: 风险信号识别准确率基于历史回测达到70%以上（过去10年危机前预警命中率）
